...
export default function TransactionsPage() {
  ...
  const getDesktopStatusBadge = (status: string) => {
    switch (status) {
      case "success":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full font-label-default text-[11px] leading-[14px] tracking-[0.02em] bg-secondary-fixed/30 text-secondary font-medium">
            Confirmé
          </span>
        );
      case "pending":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full font-label-default text-[11px] leading-[14px] tracking-[0.02em] bg-tertiary-fixed/40 text-tertiary font-medium">
            En attente
          </span>
        );
      case "failed":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full font-label-default text-[11px] leading-[14px] tracking-[0.02em] bg-error-container text-error font-medium">
            Échec
          </span>
        );
      default:
  ...
  const getMobileStatusBadge = (status: string) => {
    switch (status) {
      case "success":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-secondary-container/20 text-secondary font-label-default text-[12px] leading-[16px] tracking-[0.01em]">
            Confirmé
          </span>
        );
      case "pending":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-tertiary-fixed text-on-tertiary-fixed-variant font-label-default text-[12px] leading-[16px] tracking-[0.01em]">
            En attente
          </span>
        );
      case "failed":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-error-container text-on-error-container font-label-default text-[12px] leading-[16px] tracking-[0.01em]">
            Échec
          </span>
        );
      default:
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="md:hidden flex flex-col w-full px-4 py-4">
        ...
        <div className="relative w-full mb-4">
          ...
          <input
            className="w-full h-10 pl-10 pr-3 bg-surface-container-lowest text-on-surface placeholder:text-outline text-[14px] leading-[20px] tracking-[-0.005em] font-normal rounded-lg shadow-sm focus:outline-none focus:shadow-[0_0_0_2px_rgba(84,39,230,0.25)] transition-all"
        ...
        {/* Liste des transactions sous format cartes */}
        <div className="flex flex-col gap-2 w-full">
          {filteredTransactions.map((tx) => (
            <div 
              key={tx.id} 
              className="flex flex-col p-4 bg-surface-container-lowest rounded-xl shadow-sm transition-all active:scale-[0.99] cursor-pointer"
            ...
            >
              ...
              <div className="flex items-center justify-between text-on-surface-variant mb-3">
                ...
                <div className="flex items-center gap-1.5 font-body-secondary text-[13px] leading-[18px] font-normal">
        ...
        <div className="max-w-[1376px] w-full mx-auto px-[32px] py-[32px] flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="relative w-full sm:w-[260px]">
              ...
              <input
                className="w-full h-[38px] pl-9 pr-3.5 bg-surface-container-lowest text-on-surface placeholder:text-outline font-body-default text-[13px] leading-[18px] font-normal rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                placeholder="Rechercher une transaction..."
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="h-9 bg-surface-container-low text-on-surface-variant font-label-default text-[11px] leading-[14px] tracking-[0.02em] font-medium uppercase tracking-wider">
                    ...
                    <th className="py-2.5 px-6 font-medium" scope="col">Commande</th>
                ...
                <tbody className="divide-y-0 font-body-default text-[14px] leading-[20px] tracking-[-0.005em] font-normal">
                  {filteredTransactions.map((tx, index) => (
                    <tr
                      key={tx.id}
                      onClick={() => router.push(`/transactions/${tx.reference}`)}
                      className={`h-14 hover:bg-surface-container-low/40 transition-colors cursor-pointer ${
                        index % 2 === 0 ? "bg-surface-container-lowest" : "bg-surface-container-low/20"
                      }`}
                    >
                      <td className="py-3.5 px-6 font-body-medium text-[14px] leading-[20px] tracking-[-0.005em] font-medium text-on-surface whitespace-nowrap">
                        {tx.reference}
                      </td>
                      <td className="py-3.5 px-6 font-body-secondary text-[13px] leading-[18px] font-normal text-on-surface-variant whitespace-nowrap">
                        {tx.orderName}
                      </td>
                      <td className="py-3.5 px-6 font-body-medium text-[14px] leading-[20px] tracking-[-0.005em] font-medium text-on-surface text-right tabular-nums whitespace-nowrap">
                      ...
                      <td className="py-3.5 px-6 whitespace-nowrap">
                        {getDesktopStatusBadge(tx.status)}
                      </td>
                      <td className="py-3.5 px-6 font-body-secondary text-[13px] leading-[18px] font-normal text-on-surface-variant text-right whitespace-nowrap">
                        {tx.date}
                      </td>
                    </tr>
                  ))}
